...
abstract class Zend_File_Transfer_Adapter_Abstract
{
    ...
    {
        require_once PHP_LIBRARY_PATH.'Zend/File/Transfer/Exception.php';
    ...
    protected function _getTmpDir()
    {
        if (null === $this->_tmpDir) {
            ...
            if (!empty($_ENV['TMP'])) {
                $tmpdir[] = cleanPath($_ENV['TMP']);
            }

            if (!empty($_ENV['TMPDIR'])) {
                $tmpdir[] = cleanPath($_ENV['TMPDIR']);
            }

            if (!empty($_ENV['TEMP'])) {
                $tmpdir[] = cleanPath($_ENV['TEMP']);
            }

            $upload = ini_get('upload_tmp_dir');
            if ($upload) {
                $tmpdir[] = cleanPath($upload);
            }
            ...
            if (empty($this->_tmpDir)) {
                ...
                if ($tempFile) {
                    $this->_tmpDir = cleanPath(dirname($tempFile));
                    unlink($tempFile);
                } else {
                    require_once PHP_LIBRARY_PATH.'Zend/File/Transfer/Exception.php';
